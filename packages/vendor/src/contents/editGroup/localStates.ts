import { VendorUserGroup, vendorUserRepository } from 'pelp-repositories/src';
import {
  atom,
  atomFamily,
  selector,
...
import { onUpdateVendorUser, userDataState } from 'src/globalStates/authState';
...
const keyName = (id: string) => `EditGroup_${id}_vendor`;

// vendorUserGroup

const vendorUserGroupState = atom<VendorUserGroup | null>({
  key: keyName('vendorUserGroup'),
  default: null
});
export const useVendorUserGroupValue = () =>
  useRecoilValue(vendorUserGroupState);
...
export const useInitialize = () =>
  useRecoilCallback(({ snapshot, set }) => async () => {
    ...
    try {
      ...
      const vendorUserGroup = await userRepository.fetchVendorUserGroup(
        vendorId
      );

      set(vendorUserGroupState, vendorUserGroup);
      set(emailsState, vendorUserGroup.emails);
      set(nameState, vendorUserGroup.name);
    } catch (e) {
      console.error(e);
    }
  });
...
export const useEditGroup = () =>
  useRecoilCallback((intf) => async () => {
    ...
    try {
      ...
      await userRepository.editVendorUserGroup({ vendorId, name });
    } catch (e) {
      console.error(e);
    ...
    snapshot.retain();
    setTimeout(() => onUpdateVendorUser(intf), 5000);
...
export const useAddEmail = () =>
  useRecoilCallback((intf) => async () => {
    ...
    try {
      ...
      await userRepository.addVendorEmails({
        vendorId,
...
// entering に戻してもう一度メールアドレスを入力し直す
...
export const useDeleteEmail = () =>
  useRecoilCallback((intf) => async (index: number) => {
    ...
    try {
      await userRepository.deleteVendorEmails({
    ...
    set(initializedState, false); // リロード
    set(snackbarOpenState, true);
    set(snackbarContentState, 'サプライヤーメールアドレスを削除しました');
    set(loadingState('deleteEmail'), false);
  });
