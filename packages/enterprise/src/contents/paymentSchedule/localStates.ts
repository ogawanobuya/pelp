...
const keyName = (id: string) => `paymenySchedule_${id}_enterprise`;
...
export const useInitialize = () =>
  useRecoilCallback(({ snapshot, set }) => async () => {
    ...
    try {
      // 1ページ目
      const res1 = await accountRepository.fetchAccountsToBePaid({
        limit: rowsPerPage
      });
      ...
      // 2ページ目
      const res2 = await accountRepository.fetchAccountsToBePaid({
        limit: rowsPerPage,
...
export const useFetchNextPage = () =>
  useRecoilCallback(({ snapshot, set }) => async () => {
    ...
    try {
      const res = await accountRepository.fetchAccountsToBePaid({
        limit: rowsPerPage,
