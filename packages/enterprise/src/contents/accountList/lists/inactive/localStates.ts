...
const keyName = (id: string) => `InactiveAccountList_${id}_enterprise`;
...
const newPayDateModalOpenState = atom<boolean>({
  key: keyName('newPayDateModalOpen'),
  default: false
});
export const useNewPayDateModalOpenState = () =>
  useRecoilState(newPayDateModalOpenState);
...
export const useEditNewPayDate = () =>
  useRecoilCallback(({ snapshot, set, reset }) => async () => {
    ...
    const accounts = await snapshot.getPromise(accountsState);
    const checked = await snapshot.getPromise(checkedState);
    const editedAccounts = accounts.filter((account, index) => checked[index]);
    const newPayDate = await snapshot.getPromise(newPayDateState);

    set(loadingState, true);
    try {
      await Promise.all(
        editedAccounts.map(async (account) => {
          await accountRepository.editAccount(account.id, {
            vendorName: account.vendorName,
            vendorEmail: account.vendorEmail,
            amount: account.amount,
            currency: account.currency,
            originalDueDate: Timestamp.fromDate(
      ...
      set(snackbarContentState, '早払い日の更新に失敗しました');
      return;
    }

    set(newPayDateModalOpenState, false);
    set(loadingState, false);
