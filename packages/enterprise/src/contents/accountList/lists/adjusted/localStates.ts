...
const isOpenSetAsPaidModalState = atom<boolean>({
  key: keyName('isOpenSetAsPaidModal'),
  default: false
});
export const useIsOpenSetAsPaidModalState = () =>
  useRecoilState(isOpenSetAsPaidModalState);
...
export const useSetAsPaidAccount = () =>
  useRecoilCallback(({ snapshot, set, reset }) => async () => {
    ...
    try {
      await accountRepository.setAsPaidAccounts(checkedAccountIds);
    } catch (e) {
      console.error(e);
      set(loadingState, false);
      set(snackbarOpenState, true);
      set(snackbarContentState, '支払い済みとして設定することに失敗しました');
      return;
    }
    set(loadingState, false);
    set(snackbarOpenState, true);
    set(snackbarContentState, '支払い済みとして設定しました');

    set(isOpenSetAsPaidModalState, false);
